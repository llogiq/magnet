...
static int magnet_read(lua_State *L) {
	...
	if (lua_type(L, 1) == LUA_TNUMBER)
		...
		n = 0; // sorry, stdin has no eof.
	do {
		char *p = luaL_prepbuffer(&b);
		if (rlen > n) rlen = n;  /* cannot read more than asked */
		nr = FCGI_fread(p, sizeof(char), rlen, FCGI_stdin);
		luaL_addsize(&b, nr);
...
int main(int argc, char **argv) {
	...
	while (FCGI_Accept() >= 0) {
		...
		if (magnet_get_script(L, scriptFilename)) {
			printf("Status: 404\r\n\r\n");
		...
		for (i = strlen(scriptFilename); i >= 0 && scriptFilename[i] != '/'; i--);
		if (i >= 0) {
			path = malloc(i + 1);
			strncpy(path, scriptFilename, i);
			path[i] = '\0';
			chdir(path);
			free(path);
		}
		...
		lua_getglobal(L, "io");
		lua_pushcfunction(L, magnet_read);
